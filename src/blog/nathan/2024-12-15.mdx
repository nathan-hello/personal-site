...
- something that is made complete obsolete by just using the filesystem (everything iCloud, AirDrop)
...
as I'm trying to get a device to be sent the correct string command from the processor, and it just won
t send. I go into the module, goto definition a couple times, pull out a handy ctrl+f, and bam! Compiled
...
it is. If you're using any major vendor, it is just as closed as iOS. And good luck disable google play
...
I want to be convinced that the above two paragraphs are wrong. I want to get a 10 year phone that runs
...
hese are your system controls, changing audio source and wifi and such, and if you want some extra buttons
...
This opinion of mine extends to desktop, by the way. For consistency's sake I will specify that I don'
...
is in the advertising and big data business. Natalie and I go back and forth on this, and I will absolutely
...
you ever seen someone use a non-default keyboard on iOS? No, because they are all just worse. There isn
t any magic to it, there's no API that apple's has that the others don't. They are just worse hands down
and yet another example of customization just being a pursuit of openness with zero benefit. I get apple
s attitude change, and I get why they want to give these little breadcrumbs so people tell their android
...
s no difference between long pressing the wifi symbol to show that page, and the same for media. And it
s worse because with multiple pages, swiping down means you go to the next page instead of closing the
