	...
	for {
		...
		db.Db().InsertMessage(
			...
			db.InsertMessageParams{
				// TODO(nate): fix this being a reference
				// normalize chats being able to have anons
				// in them via chatroom id. This is already
				// happening with newMessageFromBytes but it
				// isn't good enough.
				AuthorID:       &user.ID,
